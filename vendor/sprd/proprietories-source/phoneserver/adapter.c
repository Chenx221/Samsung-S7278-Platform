...
int adapter_pty_write(pty_t * pty, char *buf, int len)
{
    ...
    if(pty == NULL || pty->ops ==NULL || buf == NULL)
        return -1;
    p_echo = strcasestr(buf, "AT");
    if(p_echo == buf) {
        if(pty->eol == 0) {
            snprintf(str, sizeof(str),"%s", buf);  //AT\r
        } else {
           snprintf(str, sizeof(str),"%s%s", buf, "\n");  //AT\r\n
        }
    } else {
...
int cvt_echupvt_set_req(AT_CMD_REQ_T * req)
{
    ...
    str_tmp = req->cmd_str;
    mux = adapter_get_cmux(req->cmd_type, TRUE);
    at_tok_start(&str_tmp, '=');
    at_tok_nextstr(&str_tmp, &str);
