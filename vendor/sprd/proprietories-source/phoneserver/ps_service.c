    ...
    if (findInBuf(input, rsp->len, "+SIPCONFIG")) {
        ...
        do {
            ...
            if ((cid <= MAX_PPP_NUM) && (cid >=1)){
                ...
                /* set property */
                snprintf(link, sizeof(link), "veth%d %s mtu 1400 netmask 255.255.255.255 up", cid-1, ip);
                property_set(SYS_IFCONFIG_UP, link);
                snprintf(link, sizeof(link), "link set veth%d arp off", cid-1);
...
int cvt_cgact_deact_req(AT_CMD_REQ_T * req)
{
    ...
    if (tmp_cid <= 3) {	/*deactivate PDP connection */
        ...
        usleep(200*1000);
         /* set property */
        snprintf(link, sizeof(link), "veth%d %s down", tmp_cid-1, "0.0.0.0");
