...
//for CSR BT Initialization
		ALOGI("Warmreset %s.",err < 0 ? "failed" : "done");
		ALOGI("[BCCMD]command done. CSR_TRANSPORT_BCSP");

		transport_close(CSR_TRANSPORT_BCSP);

		ALOGI("[BCCMD]transport_close done. CSR_TRANSPORT_BCSP");
		...
		ALOGI("[BCCMD]start hciattach done.");
...
static int bt_power_turn(int on) {

	int i,fd;
	char rfkill_path[96]={0,}, buf[10],bOn;

	for(i=0;i<10;i++)
	{
		sprintf(rfkill_path, "/sys/class/rfkill/rfkill%d/type", i);
		...
		if(read(fd, &buf, sizeof(buf)) > 8)
		{
			...
			{
				sprintf(rfkill_path, "/sys/class/rfkill/rfkill%d/state", i);
				close(fd);
				ALOGI("found rfkill for bluetooth index=%d",i);
				break;
			}
		}
		close(fd);
...
{
	char *device = NULL;
	int i, err, opt, transport = CSR_TRANSPORT_HCI, count =0;
	...
	if (transport_open(transport, device, bcsp_rate) == 0 )
		...
		if(count>10)
		{
			ALOGI("bcsp open fail\n");
			exit(1);
		}
		ALOGI("bcsp open fail, retry\n");
		bt_power_turn(0);
		usleep(100000);
		bt_power_turn(1);
		count++;
...
void read_default_bdaddr(int transport, uint16_t stores)
{
    ...
    char bdaddr[18];
    char bdaddr4bttxt[24];
    int len = 17;
    int len4bttxt = 23;
    int i, j, k;
    memset(bdaddr, 0, (len + 1) * sizeof(char));
    memset(bdaddr4bttxt, 0, (len4bttxt + 1) * sizeof(char));
    ...
    if (!fd) {
        fprintf(stderr, "open(/efs/bluetooth/bt_addr) failed: %s (%d)\n", strerror(errno), errno);

        fd = fopen("/efs/imei/bt.txt", "r");
        if (!fd) {
            fprintf(stderr, "open(/efs/imei/bt.txt) failed: %s (%d)\n", strerror(errno), errno);
            return;
        } else {
            sz = fread(bdaddr4bttxt, 1, len4bttxt, fd);
            bdaddr4bttxt[len4bttxt]='\0';
            if (sz < 0) {
                fprintf(stderr, "read(/efs/imei/bt.txt) failed: %s (%d)\n", strerror(errno), errno);
                fclose(fd);
                return;
            } else if (sz != len4bttxt) {
                fprintf(stderr, "read(/efs/imei/bt.txt) unexpected size %d\n", sz);
                fclose(fd);
            return;
            }

            memset(bdaddr, ':', (len + 1) * sizeof(char));

            for (i =0, j = 0, k =0; i < 12; i++, j++) //12: bt.txt bt address cnt
            {
                if ((j % 3) == 2) //true: ':'
                {
                    j = i + k + 1;
                    *(bdaddr + j) = * (bdaddr4bttxt + i + 11); //11: bt.txt bt_macaddr: cnt
                    k++;
                } else {
                    *(bdaddr + j) = * (bdaddr4bttxt + i + 11); //11: bt.txt bt_macaddr: cnt
                }
        ...
        bdaddr[len]='\0';
        if (sz < 0) {
            fprintf(stderr, "read(/efs/bluetooth/bt_addr) failed: %s (%d)\n", strerror(errno), errno);
            fclose(fd);
            return;
        } else if (sz != len) {
            fprintf(stderr, "read(/efs/bluetooth/bt_addr) unexpected size %d\n", sz);
            fclose(fd);
            return;
        }
        fclose(fd);
