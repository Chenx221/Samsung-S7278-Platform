...
// SSBT :: SISO
#define GATT_DEVICE_INTERFACE "org.bluez.GattDevice"
#define GATT_CONNECT_TIMEOUT 5
...
// SSBT :: SISO LE (02/22/2012)
	uint8_t *client_char_desc;
	size_t client_desc_len;
...
// SSBT :: SISO LE (02/22/2012)
	if (chr->client_char_desc)
		dict_append_array(&dict, "ClientDescriptor", DBUS_TYPE_BYTE, &chr->client_char_desc,
										chr->client_desc_len);
...
// SSBT :: SISO LE (02/22/2012)
static void gatt_write_char_req_cb(guint8 status, const guint8 *pdu, guint16 plen,
							gpointer user_data)
{
	struct query_data *qdata = user_data;
	struct gatt_service *gatt = qdata->gatt;
	struct characteristic *chr = qdata->chr;
	DBusMessage *reply;
	DBusMessage *msg;
	...
	DBG("Client : gatt_write_char_req_cb, status = %d", status);
	btd_device_remove_attio_callback(gatt->dev, gatt->attioid);
	gatt->attioid = 0;
	...
	if(status == 0){
		...
		if (!dec_write_resp(pdu, plen)) {
			...
			returnStr = "Characteristic value was written sucessfully";
			DBG("Characteristic value was written sucessfully");
		}
		...
		reply = btd_error_invalid_args(qdata->msg);
		if (!reply) {
			DBG("reply is NULL");
			qdata->msg = NULL;
		} else {
	...
	emit_property_changed(gatt->conn, chr->path,CHAR_INTERFACE, 
		                        "Value", DBUS_TYPE_STRING , &returnStr);
}

static void gatt_char_written(gpointer user_data)
{
	struct characteristic *chr = user_data;
	struct gatt_service *gatt = chr->gatt;
...
// SSBT :: SISO LE (02/22/2012)
static DBusMessage *set_value(DBusConnection *conn, DBusMessage *msg,const char* methodType,
...
// SSBT :: SISO LE
	if (gatt->attrib) {
		if (g_str_equal("WRITE_CMD",methodType)) {
			DBG("set_value WRITE_CMD");
			gatt_write_cmd(gatt->attrib, chr->handle, value, len,
											gatt_char_written, chr);
		} else if(g_str_equal("WRITE_REQ",methodType)) {
			DBG("set_value WRITE_REQ");
			qdata = g_new0(struct query_data, 1);
			qdata->gatt = chr->gatt;
			qdata->chr = chr;
			qdata->msg = dbus_message_ref(msg);
			gatt_write_char(gatt->attrib, chr->handle, value, len,
											gatt_write_char_req_cb, qdata);
		}			
	} else {
...
// SSBT :: SISO LE (02/22/2012)
static int characteristic_set_client_conf(struct characteristic *chr,
					const uint8_t *value,  size_t vlen)
{
	uint8_t *char_desc;
	char_desc = g_try_realloc(chr->client_char_desc, vlen);
	if(char_desc == NULL)
		return -ENOMEM;
	chr->client_char_desc = char_desc;
	memcpy(chr->client_char_desc, value, vlen);
	chr->client_desc_len = vlen;
	return 0;
}

static void gatt_write_client_config_desc_resp(guint8 status, const guint8 *pdu,
					guint16 len, gpointer user_data)
{
	...
	if (status == 0) {
		...
		characteristic_set_client_conf(chr, value, vlen); //Patch for unpair crash
			...
			DBusMessageIter *iter, struct characteristic *chr)
{
	...
	gatt_write_char(gatt->attrib, chr->client_char_desc_handle, value, len,
					gatt_write_client_config_desc_resp, qvalue);
					...
					DBusMessage *msg, void *data)
{
	...
	DBusMessageIter sub;
// SSBT :: SISO LE (02/22/2012)
	const char *property,*methodType;
...
// SSBT :: SISO LE (02/22/2012)
	...
	dbus_message_iter_get_basic(&iter, &methodType);
	dbus_message_iter_next(&iter);
...
// SSBT :: SISO LE (02/22/2012)
	if (g_str_equal("Value", property)) {
		return set_value(conn, msg, methodType,&sub, chr);
	} else if (g_str_equal("ClientConfiguration", property)) {
		return set_client_config_desc(conn, msg, &sub, chr);
	}
...
static void register_characteristic(gpointer data, gpointer user_data)
{
	...
	if (g_dbus_register_interface(conn, chr->path, CHAR_INTERFACE,
					char_methods, NULL, NULL, chr, NULL) !=TRUE) {
		DBG("Char Registered failed : %s", chr->path);
	}//Added if condition to check result of register_interface- Prevent defect issue
...
// SSBT :: SISO LE (02/22/2012)
static void update_client_char_config (guint8 status, const guint8 *pdu, guint16 len,
								gpointer user_data)
{
	struct query_data *current = user_data;
	...
	chr->client_char_desc = g_malloc(len - 1);
	
	if (chr->client_char_desc == NULL) {
		DBG("client_char_desc = NULL so return");
		return;
	}

	memcpy(chr->client_char_desc, pdu + 1, len - 1);
	
	chr->client_desc_len = len - 1;   
	chr->client_char_desc_handle = current->handle;


	store_attribute(gatt, current->handle, GATT_CLIENT_CHARAC_CFG_UUID,
				(void *)chr->client_char_desc, len);
...
// SSBT :: SISO
			...
			//to do 
			gatt_read_char(gatt->attrib, handle, 0,
						update_client_char_config, qfmt);
		} else
			g_free(qfmt);
	}
							...
							gpointer user_data)
{
	...
	bdaddr_t sba, dba;
	char* value = NULL;
	const char* cmpValue = "0xffff"; // value for discover alll chars 
	...
	//SISO-check to send related charpaths incase discovercharbyUuid was called
	if (dbus_message_get_args(gatt->query->msg, NULL, DBUS_TYPE_STRING, &value,
					DBUS_TYPE_INVALID) == FALSE) {
					btd_error_invalid_args(gatt->query->msg);
					return ;
	}
	...
	//SISO-check to send related charpaths incase discovercharbyUuid was called
	if (g_strcmp0(value, cmpValue) == 0) {
		DBG("******* discovered for all Chars****** ");
		...
		DBG("******* discovered for char by UUUID****** ");
		for (l = gatt->chars; l; l = l->next) {
			struct characteristic *chr = l->data;
			if (g_strcmp0(value,chr->type) == 0) {
				dbus_message_iter_append_basic(&array_iter,
					DBUS_TYPE_OBJECT_PATH, &chr->path);
				break;
			}
...
static void send_discover(GAttrib *attrib, gpointer user_data)
{
	...
	DBG("");
       ...
       //SISO_BLUEZ_BT_LE_FW modification 02/15/2012	
	char* value = NULL;
	const char* cmpValue = "0xffff"; //value for discover alll chars 
...
// SSBT :: SISO LE (02/22/2012)
	...
	if (g_strcmp0(value, cmpValue) == 0) {
		DBG("send_discover for all Chars");
		gatt_discover_char(gatt->attrib, prim->start, prim->end, NULL,
						char_discovered_cb, qchr);
	} else {	
		DBG("send_discover char by UUUID");
		bt_uuid_t charUuid;
		bt_string_to_uuid(&charUuid, value);
		gatt_discover_char(gatt->attrib, prim->start, prim->end, &charUuid,
						char_discovered_cb, qchr);
	}
}
...
// SSBT :: SISO LE
	{ "DiscoverCharacteristics",	"s",	"ao",	discover_char,
					G_DBUS_METHOD_FLAG_ASYNC	},
						...
						int psm)
{
	...
	if(g_dbus_register_interface(gatt->conn, gatt->path,
					...
					NULL, NULL, gatt, NULL) != TRUE) {
		DBG("Register interface failed for gatt->path = %s",gatt->path);
	}//Added if condition to check the result of register_interface -Prevent defect issue
...
// SSBT :: KJH + (0227), temp to remove callback while connection/disconnection. should be checked
void remove_all_attio_callback ()
{
	GSList *l, *left, *temp;
	struct query *query;
	struct gatt_service *gatt;
	...
	for (l = gatt_services, left = NULL; l; l = l->next) {
		...
		if (query) {
			temp = g_slist_last (query->list);
			qdata = temp->data;

			while(temp) {
				query->list = g_slist_remove(query->list, qdata);
				temp = g_slist_last (query->list);
