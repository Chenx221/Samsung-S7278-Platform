						...
						DBusMessage *msg, void *data)
{
	...
	nc->io = bt_io_connect(BT_IO_L2CAP, connect_cb, nc,
				...
				/*
				* SS_BLUETOOTH(insick.jung) 2012.11.05
				* AT&T 10776 test fail. The unpaired device is added to the list of paired devices without any notifications to DUT.
				* Set outgoing security level to high.
				*/
				BT_IO_OPT_SEC_LEVEL, BT_SECURITY_HIGH,
				BT_IO_OPT_INVALID);
	if (!nc->io) {
		DBusMessage *reply;
		error("%s", err->message);
