...
#if (BOARD_HAVE_CSR_FM) || (SEC_FMRADIO_MRVL)
...
static int mgmt_set_powered(int index, gboolean powered)
{
	...
	*data = index;
	...
	if(powered)
	{
		...
		/* Schedule a MGMT_OP_SET_POWERED  complete  in 'timeout' milliseconds */
		info->hciup_timer_id = g_timeout_add(10, hciup_timout_handler, data);
	}
	else
	{
		DBG("start_hcidown_timer: hci%d", index);
		...
		/* Schedule a MGMT_OP_SET_POWERED  complete  in 'timeout' milliseconds */
		info->hcidown_timer_id = g_timeout_add(10, hcidown_timout_handler, data);
	}
...
static int mgmt_set_scopath(int index, uint32_t sco_path)
{
	...
	}else if(sco_path ==3){//i2s_Disable -> PCM set
		buf[0] = 0x00; //i2s_Disable, 0x0
		len = 4;

		if (hci_send_cmd(dd, OGF_VENDOR_CMD, OCF_WRITE_I2SPCM_INTERFACE_PARAM, len, buf) < 0){
			DBG("hci%d set_scopath (%d) : OCF_WRITE_I2SPCM_INTERFACE_PARAM error ", dd, sco_path);
			err = -errno;
			goto fail;
		}
