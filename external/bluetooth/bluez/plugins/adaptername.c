...
// SSBT :: NEO (0207) + : for android				
			...
			case 'n':
				...
				opt = value;
				break;
//Property added to get customised BT device name from init.smdk4210.rc instead of using default model name as BT device name.
                        case 'a':
                                property_get("ro.bt.adaptername", value, "");
                                opt = value;
                                break;
#endif
								...
								gpointer data)
{
	...
	while (i < len) {
		...
		/*Prevent-defect issue: pevent->name!=NULL cond is removed
		*in below if block
		*/
		if (pevent->len && strcmp(pevent->name, MACHINE_INFO_FILE) == 0)
			changed = TRUE;
