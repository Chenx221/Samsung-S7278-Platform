...
#define G_FM_STATE "brcm.fm_state"
#define G_BT_STATE "brcm.bt_state"
// this value should be same as defined in cutils/properties.h
...
#ifdef BOARD_HAVE_BLUETOOTH_BCM
/* Broadcom added code to detect the BT/FM state to help decide whether
 * to inform the frameworks when quick-switch is enabled.
 */
static int get_fm_state(void) {
    int ret;
    char state[3];
    memset(state, 0 ,3);
    property_get(G_FM_STATE, state, "0");
    ret = atoi(state);
    printf("get_fm_state%d", ret);
    return ret;
}

static int set_fm_state(int state)
{
    int ret;
    char value[PROPERTY_VALUE_MAX];
    memset(value, 0 ,PROPERTY_VALUE_MAX);
    sprintf(value, "%d", state);
    ret = property_set(G_FM_STATE, value);
    printf("set_fm_state%d, ret==%d, value]=%s", state, ret, value);
...
static int get_bt_state(void) {
    ...
    char state[3];
    memset(state, 0 ,3);
    property_get(G_BT_STATE, state, "0");
    ret = atoi(state);
    printf("get_bt_state%d", ret);
    return ret;
}

static int set_bt_state(int state)
{
    int ret;
    char value[PROPERTY_VALUE_MAX];
    memset(value, 0 ,PROPERTY_VALUE_MAX);
    sprintf(value, "%d", state);
    ret = property_set(G_BT_STATE, value);
    printf("set_bt_state%d, ret==%d, value]=%s", state, ret, value);
    return 0;
}
#endif
...
static void link_key_notify(int index, void *ptr)
{
	...
	if (key_type < 0x03 || (key_type == 0x06 && old_key_type != 0xff) ||
		...
		/* SS_BLUETOOTH(gudam.ryu) 2012. 02. 29 - fixed for opp sending fail issue */
		else if (err == 0) 
			persistent = TRUE;
...
/* SS_BLUETOOTH(changeon.park) 2012.02.28 */
//[P120224-0490] changed for fixing pairing err with certain heasets
#ifndef GLOBALCONFIG_BLUETOOTH_USE_CSR
	g_timeout_add_seconds_full(G_PRIORITY_DEFAULT, 1, __get_remote_version,
								req, g_free);
#else
       g_timeout_add_full(G_PRIORITY_DEFAULT, 10, __get_remote_version,
								req, g_free);
#endif
...
#ifdef BOARD_HAVE_BLUETOOTH_BCM
void device_event(int event, int index)
{
	int bt_state = get_bt_state();
	info("%s: bt_sate = %d", __FUNCTION__, bt_state);
	switch (event) {
	case HCI_DEV_REG:
	...
	case HCI_DEV_DOWN:
		if (bt_state == DISABLING_STATE) {
			set_bt_state(OFF_STATE);
...
#ifdef BOARD_HAVE_BLUETOOTH_BCM
/* Bluez will not work properly when connect & disconnect with headset very quickly.
 * It looks like bluez kernel does not work properly when connect & disconnect quickly,
 * so here disconnect all ACL connection intentional, to make the repetition rate much less.
 */
#ifndef HCI_DEV_ID
#define HCI_DEV_ID  0
#endif

static int cls_conn(void)
{
	struct hci_conn_list_req *cl;
	struct hci_conn_info *ci;
	int i,dd;
	...
	cl->dev_id = HCI_DEV_ID;
	cl->conn_num = 10;
	ci = cl->conn_info;

	if (ioctl(dd, HCIGETCONNLIST, (void *) cl)) {
		info("Can't get connection list");
		goto out;
	}
...
#endif
		if (ci->handle) {
			if (hci_disconnect(dd, ci->handle,HCI_OE_USER_ENDED_CONNECTION, 10000)<0) {
				info("hci_disconnect fail errno==%d", errno);
			}
		} else {
			if (ci->state == BT_CONNECT || ci->state == BT_CONNECT2) {
				create_conn_cancel_cp cp;
				memset(&cp, 0, sizeof(cp));
				memcpy(&cp, &ci->bdaddr, sizeof(cp));

				if (hci_send_cmd(dd, OGF_LINK_CTL, OCF_CREATE_CONN_CANCEL, CREATE_CONN_CANCEL_CP_SIZE, &cp) < 0) {
					info("hci_send_cmd fail errno==%d", errno);
				}
			}
		}
	}
...
static int hciops_set_powered(int index, gboolean powered)
{
	...
	if (powered == FALSE){
		...
		if (fm_state == OFF_STATE) {
			...
			set_bt_state(DISABLING_STATE);
			device_event(HCI_DEV_DOWN, index);
			return 0;
		}
	} else {
		set_bt_state(ENABLING_STATE);
		if (fm_state == OFF_STATE) {
