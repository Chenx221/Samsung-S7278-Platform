...
#ifdef GLOBALCONFIG_BLUETOOTH_REMOVE_PAIRED_LIST_TO_UPGRADE
#include <stdlib.h>
#include <sys/ioctl.h>
#include <sys/file.h>
#endif
...
#ifdef GLOBALCONFIG_BLUETOOTH_REMOVE_PAIRED_LIST_TO_UPGRADE
	int fd;
	int check_file_created = 0;

	fd = open("/data/misc/bluetoothupg404", O_RDONLY);
	if (fd < 0) {
		DBG("create bluetoothupg404");
		check_file_created = 1;
		system("rm -r /data/misc/bluetoothd");
	} else {
		close(fd);
	}
#endif
...
#ifdef GLOBALCONFIG_BLUETOOTH_REMOVE_PAIRED_LIST_TO_UPGRADE
	if (check_file_created != 0) {
		system("touch /data/misc/bluetoothupg404");
	}
#endif
