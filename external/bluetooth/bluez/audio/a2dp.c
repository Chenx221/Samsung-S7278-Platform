...
static gboolean select_capabilities(struct avdtp *session,
        struct avdtp_remote_sep *rsep, GSList **caps,struct sink *sink)
#else
static gboolean select_capabilities(struct avdtp *session,
 ...
 {
    DBG(" executing select capabilities when both aptx and scmst is enabled statically ...");
    struct avdtp_service_capability *media_transport, *media_codec;

    /** +++ Prejith SCMST feature added for APT-X  */
    #ifdef GLOBALCONFIG_BT_SCMST_FEATURE
    struct avdtp_service_capability *media_content_protection;
	...
	*caps = g_slist_append(*caps, media_transport);
	...
	if (aptx_found == TRUE) {
		...
		media_codec = avdtp_service_cap_new(AVDTP_MEDIA_CODEC, &sbc_cap,
							sizeof(sbc_cap));
	*caps = g_slist_append(*caps, media_codec);
            /** +++ Prejith If both APT-X SCMST feature enabled  */
    #ifdef GLOBALCONFIG_BT_SCMST_FEATURE
